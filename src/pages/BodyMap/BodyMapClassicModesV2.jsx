...
export default function BodyMapClassicModesV2() {
  ...
  // Active mode's regions on the model
  const regionStyles = {}
  const labels = {}
  Object.entries(sel[activeMode]).forEach(([id, d]) => {
    const isActive = active && active.mode === activeMode && active.partId === id
    regionStyles[id] = { fill: hexToRgba(color, severityAlpha(d.severity)), stroke: color, strokeWidth: isActive ? 2 : 1 }
  ...
  // ---- Done / submitted summary ----
  if (submitted) {
    return (
      <Box sx={{ p: 4 }}>
        <Box sx={{ maxWidth: 720 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 2, mb: 3, borderRadius: 'var(--radius-md)', backgroundColor: 'var(--color-background-secondary)', border: '1px solid var(--color-success)' }}>
            <Box sx={{ width: 28, height: 28, borderRadius: '50%', backgroundColor: 'var(--color-success)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, flexShrink: 0 }}>✓</Box>
            <Box>
              <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>Responses submitted</Typography>
              <Typography variant="body2" sx={{ color: 'var(--color-text-secondary)' }}>{total} entr{total === 1 ? 'y' : 'ies'} across {MODES.filter((m) => Object.keys(sel[m]).length).length} symptom type{MODES.filter((m) => Object.keys(sel[m]).length).length === 1 ? '' : 's'}.</Typography>
            </Box>
          </Box>
          {[['FRONT', frontEntries], ['BACK', backEntries]].map(([heading, list]) => list.length > 0 && (
            <Box key={heading} sx={{ mb: 2 }}>
              <Typography variant="caption" sx={{ display: 'block', color: 'var(--color-text-muted)', fontSize: '11px', fontWeight: 700, letterSpacing: '0.06em', mb: 1 }}>{heading}</Typography>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {list.map((e) => {
                  const c = MODE_COLORS[e.mode]
                  return (
                    <Box key={`${e.mode}::${e.partId}`} sx={{ display: 'inline-flex', alignItems: 'center', gap: 1, height: 30, pl: 0.5, pr: 1.5, borderRadius: '15px', backgroundColor: hexToRgba(c, 0.12), border: `1px solid ${c}` }}>
                      <Box sx={{ width: 20, height: 20, borderRadius: '50%', backgroundColor: c, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700 }}>{e.severity}</Box>
                      <Typography variant="body2" sx={{ color: c, fontSize: '12px', fontWeight: 500 }}>{e.name} · {e.mode}</Typography>
                    </Box>
                  )
                })}
              </Box>
          ...
          <Button variant="outlined" onClick={() => setSubmitted(false)} sx={{ mt: 1, textTransform: 'none', fontWeight: 500, borderColor: 'var(--color-border-primary)', color: 'var(--color-primary)' }}>Edit responses</Button>
        </Box>
      </Box>
    )
  }
  ...
  return (
    <Box sx={{ p: 4 }}>
      <Box sx={{ maxWidth: 960 }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mt: 1 }}>Body map for Sprint Review</Typography>
          ...
          <Box sx={{ flex: '1 1 360px', minWidth: 300 }}>
            ...
            {total === 0 && !activeData ? (
              ...
              <Box>
                ...
                {/* Done */}
                <Button variant="contained" disableElevation onClick={() => setSubmitted(true)}
                  sx={{ mt: 1, px: 3, py: 1, textTransform: 'none', fontWeight: 500, backgroundColor: 'var(--color-primary)', color: '#fff', '&:hover': { backgroundColor: 'var(--color-primary-hover)' } }}>
                  Done
                </Button>
              </Box>
            )}
