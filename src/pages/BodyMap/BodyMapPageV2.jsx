...
function BuilderConfig({ config, onChange }) {
  const { instructions, bodyDisplay, symptomTypes, allowComment, customQuestion } = config
    ...
    <Paper elevation={0} sx={{ border: '1px solid var(--color-border-primary)', borderRadius: 'var(--radius-md)', p: 3 }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
        ...
        <Box>
          ...
          <FormHelperText sx={{ mt: 1, mx: 0, color: 'var(--color-text-secondary)' }}>
  ...
  return (
    <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-start' }}>
      ...
      <Box sx={{ flex: '0 0 auto', width: 420, display: 'flex', flexDirection: 'column', gap: 2 }}>
        {/* Subsection label + question heading + instruction text (Change 5) */}
      ...
      <Box sx={{ flex: '0 0 220px' }}>
        {/* Single flip pill button — top right of the model (desktop) */}
...
export default function BodyMapPageV2() {
