...
import { Box, Typography, Button, Slider, TextField, Collapse, IconButton, Drawer } from '@mui/material'
...
// ---- Section navigation (collapsed strip + expandable drawer) -------------
const NAV_SECTIONS = [
  {
    title: 'Section 1',
    completed: '1 of 1 steps completed',
    subs: [{ label: 'Sub-section 1.1', active: true }],
  },
]
...
function SideNav({ open, onOpen, onClose, container, disabled }) {
  return (
    <>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', px: 1, py: 1, borderBottom: '1px solid var(--color-border-primary)' }}>
        ...
        <Box sx={{ overflowY: 'auto' }}>
          {NAV_SECTIONS.map((section) => (
            <Box key={section.title}>
              <Box sx={{ display: 'flex', alignItems: 'flex-start', px: 1.5, py: 1.5, backgroundColor: 'var(--color-background-secondary)' }}>
                <DragIndicator sx={{ fontSize: 18, color: 'var(--color-text-disabled)', mr: 1, mt: 0.25 }} />
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  <Typography variant="body2" sx={{ fontWeight: 600, color: 'var(--color-text-primary)', fontSize: '13px' }}>{section.title}</Typography>
                  <Typography variant="caption" sx={{ color: 'var(--color-text-secondary)', fontSize: '11px' }}>{section.completed}</Typography>
                </Box>
              ...
              {section.subs.map((sub) => (
                <Box
                  key={sub.label}
                  sx={{ display: 'flex', alignItems: 'center', pl: 3, pr: 1.5, py: 1.25, backgroundColor: sub.active ? 'var(--color-background-secondary)' : 'transparent', borderBottom: '1px solid var(--color-border-secondary)' }}
                >
                  <DragIndicator sx={{ fontSize: 16, color: 'var(--color-text-disabled)', mr: 1 }} />
                  <Typography variant="body2" sx={{ flex: 1, color: 'var(--color-text-secondary)', fontSize: '12px', fontWeight: sub.active ? 600 : 400 }}>{sub.label}</Typography>
...
// ---- Page ------------------------------------------------------------------
export default function BodyMapMobileClassicModesV2() {
  ...
  // sticky: hide common shortcuts permanently once a selection is confirmed (Change 1)
  const [hasConfirmed, setHasConfirmed] = useState(false)
  // side navigation (Change 2)
  ...
  return (
    <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-start', py: 4, px: 2, width: '100%' }}>
      ...
      <Box sx={{ flexShrink: 0, width: 393 + 24, height: 852 + 24, p: '12px', borderRadius: '62px', backgroundColor: '#0a0a0a', boxShadow: '0 24px 60px rgba(0,0,0,0.35)' }}>
        <Box ref={setScreenEl} sx={{ position: 'relative', width: 393, height: 852, borderRadius: '50px', overflow: 'hidden', backgroundColor: 'var(--color-background-primary)', display: 'flex', flexDirection: 'column' }}>
          {/* Dynamic island */}
          ...
          {/* Scrollable form page — inset right to clear the side-nav strip */}
          <Box sx={{ flex: 1, minHeight: 0, overflowY: 'auto', pt: 2, pr: 2, pl: '40px', pb: totalAll > 0 ? '96px' : 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            {/* Sub-section label */}
            ...
            {/* Common shortcuts — first-time guidance only; hidden once a selection is confirmed (Change 1) */}
            {!hasConfirmed && (
            <Box sx={{ mt: 0.5 }}>
          ...
          {/* Fixed Done footer — inset for the side-nav strip */}
          {totalAll > 0 && (
            <Box sx={{ position: 'absolute', bottom: 0, left: '28px', right: 0, p: 2, backgroundColor: 'var(--color-background-primary)', borderTop: '1px solid var(--color-border-primary)', zIndex: 20 }}>
          ...
          {/* Collapsed side navigation + expandable drawer (Change 2) */}
          <SideNav
            open={navOpen}
