...
function AiSummaryDrawer({ open, onClose, initialSummary = false }) {
  ...
  useEffect(() => {
    if (open) {
      resetAll()
      if (initialSummary) setSummary(true)
    }
  ...
  const visibleAthletes = useMemo(() => {
    ...
    const list = selectorAthletes.filter((a) => {
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          <Box sx={{ flex: 1, overflowY: 'auto' }}>
            {generating ? (
              ...
              /* AI SUMMARY RESULT */
              <Box sx={{ p: 2.5 }}>
                {repeatEnabled && !scheduleBannerDismissed && (
                  <Alert
                    icon={<ScheduleOutlined fontSize="small" />}
                    severity="info"
                    onClose={() => setScheduleBannerDismissed(true)}
                    sx={{
                      mb: 2,
                      py: 0,
                      alignItems: 'center',
                      ...
                      '& .MuiAlert-icon': { color: 'var(--color-ai-purple)' },
                    }}
                  >
                    This summary is also scheduled — next run {firstRunDate} at {firstRunTime}
                    <Link
                      component="button"
                      onClick={() => {
                        setSummary(null)
              ...
              <Stack divider={<Divider />}>
                ...
                <Box>
                  ...
                  <Collapse in={activeStep === 2}>
                    <Box sx={{ px: 2, pb: 2 }}>
                      ...
                      {/* Selected count chip — matches the player selector exactly */}
                      <Chip
                        label={`Selected (${selectedForms.size})`}
                        size="small"
                        sx={{
                          mb: 1.5,
                          bgcolor: 'var(--color-primary)',
                ...
                <Box sx={{ px: 2, py: 2 }}>
                  <MuiButton
                    ...
                    sx={greySecondaryBtnSx}
                  >
                    Add schedule
                  </MuiButton>

                  <Collapse in={repeatEnabled}>
                    <Stack spacing={2} sx={{ mt: 1.5 }}>
                      ...
                      <TextField
                        ...
                        placeholder="e.g. Weekly Medical Review"
                        value={schedTitle}
                      ...
                      <TimePicker
                        ...
                        onChange={(v) => setSchedStartTime(v)}
                        slotProps={{
                          textField: { size: 'small', variant: 'filled', fullWidth: true, sx: fieldStyles },
                        }}
                      />

                      {/* Recurrence — always-visible dropdown */}
                      <Box>
                        <FormControl size="small" variant="filled" fullWidth sx={fieldStyles}>
                          <InputLabel
                            sx={{
                              fontFamily: 'var(--font-family-primary)',
                              fontSize: 'var(--font-size-sm)',
                          ...
                          <Select
                            ...
                            sx={{ fontFamily: 'var(--font-family-primary)', fontSize: 'var(--font-size-sm)' }}
                          >
                            <MenuItem value="none" sx={{ fontFamily: 'var(--font-family-primary)', fontSize: 'var(--font-size-sm)' }}>Does not repeat</MenuItem>
                            <MenuItem value="daily" sx={{ fontFamily: 'var(--font-family-primary)', fontSize: 'var(--font-size-sm)' }}>Daily</MenuItem>
                            <MenuItem value="weekly" sx={{ fontFamily: 'var(--font-family-primary)', fontSize: 'var(--font-size-sm)' }}>Weekly</MenuItem>
                            <MenuItem value="everyday" sx={{ fontFamily: 'var(--font-family-primary)', fontSize: 'var(--font-size-sm)' }}>Every {recurrenceDayName}</MenuItem>
                            <MenuItem value="custom" sx={{ fontFamily: 'var(--font-family-primary)', fontSize: 'var(--font-size-sm)' }}>Custom</MenuItem>
                          </Select>
                        </FormControl>
                        ...
                        {recurrence === 'custom' && (
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1.5 }}>
                            <Typography sx={labelSx}>Every</Typography>
                            <TextField
                              type="number"
                              size="small"
                              variant="filled"
                      ...
                      {/* Notify by (lives inside the schedule section) */}
                      <Box>
                        <Typography sx={{ ...mutedSx, mb: 0.75 }}>Notify by</Typography>
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          {['Email', 'SMS', 'In-app'].map((ch) => (
                            <PillToggle
                              key={ch}
                              label={ch}
                              selected={notifyChannels.has(ch)}
            ...
            {summary ? (
              <Stack direction="row" spacing={1.5}>
                ...
                <MuiButton
                  ...
                  sx={navyBtnSx}
                >
                  <RefreshOutlined sx={{ fontSize: 16, mr: 0.5 }} /> Regenerate
                </MuiButton>
              </Stack>
            ) : (
              <Button
              ...
              >
                ...
                {repeatEnabled ? 'Create & schedule' : 'Generate summary'}
              </Button>
            )}
          </Box>
          </>
